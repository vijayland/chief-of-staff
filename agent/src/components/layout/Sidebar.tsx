...
import {
  MessageSquare, Mail, Calendar, Brain, LogOut,
  Sparkles, X, Plus, Trash2, Loader2,
} from "lucide-react";
...
export function Sidebar({ onClose }: SidebarProps) {
  ...
  useEffect(() => {
    api.auth.me().then(setUser).catch(() => null).finally(() => setLoadingUser(false));
  }, []);

  useEffect(() => {
    if (!isChat) return;
    setLoadingConvs(true);
    api.chat.conversations().then(setConversations).catch(() => null).finally(() => setLoadingConvs(false));
  ...
  function startNewChat() {
    ...
    setConversations((prev) =>
      prev.some((c) => c.id === NEW_ID)
        ? prev
        : [{ id: NEW_ID, title: "New chat", created_at: new Date().toISOString() } as Conversation, ...prev]
    );
    ...
    <aside className="w-64 lg:w-56 shrink-0 h-screen flex flex-col bg-surface-sidebar border-r border-border">
      ...
      <div className="px-4 pt-5 pb-4 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[13px] font-bold text-text-primary leading-tight">Chief of Staff</p>
            <p className="text-[10px] text-text-muted leading-tight">AI Assistant</p>
          </div>
        </div>
        {onClose && (
        ...
        <div className="space-y-0.5">
          {NAV.map(({ href, icon: Icon, label }) => {
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 shrink-0 ${active ? "text-text-primary" : "text-text-muted"}`} />
                {label}
              </Link>
            );
          })}
        ...
        <div className="flex-1 flex flex-col overflow-hidden px-3 pt-3">
          ...
          <div className="flex-1 overflow-y-auto -mx-1">
            {loadingConvs ? (
              ...
              <p className="text-center text-[11px] text-text-muted pt-4 px-3">No conversations yet</p>
            ) : (
              <div className="px-1">
                {conversations.map((conv) => {
                  const isActive = activeConvId === conv.id || (activeConvId === NEW_ID && conv.id === NEW_ID);
                  const isPending = conv.id === NEW_ID;
                  return (
                    <div key={conv.id}
                      role="button" tabIndex={0}
                      onClick={() => { if (!isPending) { setActiveConvId(conv.id); onClose?.(); } }}
                      onKeyDown={(e) => e.key === "Enter" && !isPending && setActiveConvId(conv.id)}
                    ...
                    >
                      <MessageSquare className={`w-3.5 h-3.5 shrink-0 ${isActive ? "text-text-primary" : "text-text-muted"}`} />
                      <span className={`flex-1 truncate text-xs font-medium ${isPending ? "text-text-muted italic" : ""}`}>
                        {conv.title ?? "Untitled"}
                      </span>
                      <button type="button" disabled={deletingId === conv.id}
        ...
        {loadingUser ? (
          ...
          <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-md">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-semibold text-text-primary truncate">{user.full_name}</p>
